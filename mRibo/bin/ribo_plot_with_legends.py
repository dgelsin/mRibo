...
def plot_avggene(inputs, paths_in, paths_out, settings, settings_plot):
    ...
    for fname in files: 
        ...
        for graph in ['startHM', 'stopHM']:
            ...
            plt.subplot(2,2,plot_num)
            plot = sns.heatmap(dataHM, cmap = "viridis", vmin = 0, vmax = hmmax, cbar=True)
    ...
    plt.savefig(path_figure + fname + '/avggene_' + name_settings + '_with_legends.pdf', dpi=400)
...
def plot_pausescore(inputs, paths_in, paths_out, settings, settings_plot):
    ...
    for fname in files:
        ...
        if aa_codon == 'aa':
            ...
            plt.tight_layout()
            plt.savefig(path_figure + fname + '/aa_pausescore' + name_settings + '_aa_pause_scores_with_legends.pdf', dpi=400)
        ...
        if aa_codon == 'codon':
            ...
            plt.tight_layout()
            plt.savefig(path_figure + fname + '/codon_pausescore' + name_settings + '_with_legends.pdf', dpi=400)
...
def plot_asymmetry_comp(inputs, paths_in, paths_out, settings):
    ...
    sns.despine(offset=5)
    plt.savefig(path_figure + fname + '/asymmetry_score' + name_settings + '.pdf', dpi=600, bbox_inches="tight") 
...
def plot_avggene_end(inputs, paths_in, paths_out, settings):
    ...
    for fname in files: 
        ...
        for graph in ['startHM', 'stopHM']:
            ...
            plt.subplot(2,2,plot_num)
            plot = sns.heatmap(dataHM, cmap = "ocean_r", vmin = 0, vmax = hmmax, cbar=True)
...
def plot_pausescore_APE_heatmap(inputs, paths_in, paths_out, settings, settings_plot):
    ...
    plt.savefig(outpath + 'pause_scores_with_legends.png', dpi=400)
...
def plot_pausescore_downstream(inputs, paths_in, paths_out, settings, settings_plot):
    ...
    for aa in aminoacids: 
        ...
        outpath = paths_out['path_figures']
        plt.savefig(outpath + aa + 'pause_wave_with_legends.png', dpi=400)
